...
def save_results(rv_1h, output_path, include_sqrt=True):
    """
    ...
    if output_path.endswith('.csv'):
        rv_1h.to_csv(output_path)
    elif output_path.endswith('.parquet'):
        rv_1h.to_parquet(output_path)
    elif output_path.endswith('.pickle'):
        rv_1h.to_pickle(output_path)
    else:
    ...
    if include_sqrt:
        ...
        sqrt_path = f"{base_path}_sqrt{os.path.splitext(output_path)[1]}"
...
def main():
    ...
    parser.add_argument('--output-file', type=str, default='data/rv1h.csv',
    ...
    # Save the results
    save_results(rv_1h, args.output_file, include_sqrt=args.include_sqrt)
    
    logger.info("1-hour realized volatility calculation completed successfully.")
