...
def create_visualizations(rv_df, pct_change_df, stats_df, output_dir, top_n=5):
    """
    ...
    for symbol in top_symbols:
        ...
        # Save figure
        fig.write_html(os.path.join(output_dir, f"{symbol}_rv_vs_pct_change.html"))
        
        logger.info(f"Created time series plot for {symbol}")
    ...
    for symbol in top_symbols:
        ...
        # Save figure
        fig.write_html(os.path.join(output_dir, f"{symbol}_scatter.html"))
        
        logger.info(f"Created scatter plot for {symbol}")
...
def main():
    ...
    create_visualizations(rv_aligned, pct_change_aligned, stats_df, args.output_dir, args.top_n)
