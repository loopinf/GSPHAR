#!/usr/bin/env python
"""
Script to plot predictions from the GSPHAR model trained with asymmetric MSE loss.
...
def plot_predictions_vs_actual(predictions, targets, dates, symbols, output_dir='plots', prefix='asymmetric_loss'):
    """
    ...
    for i, symbol in enumerate(symbols):
        ...
        plt.plot(dates, symbol_predictions, label='Predicted (Asymmetric Loss)', color='red', alpha=0.7)
    ...
    plt.plot(dates, avg_predictions, label='Average Predicted (Asymmetric Loss)', color='red', linewidth=2)
...
def main():
    ...
    Main function.
    """
    parser = argparse.ArgumentParser(description='Plot predictions from the GSPHAR model trained with asymmetric MSE loss.')
    ...
    parser.add_argument('--prefix', type=str, default='asymmetric_loss',
    ...
    if args.model_path is None:
        model_path = os.path.join(settings.MODEL_DIR, f"GSPHAR_20_h{args.horizon}_{args.method}_asymmetric_mse_asymmetric_loss_latest_best.pt")
    else:
